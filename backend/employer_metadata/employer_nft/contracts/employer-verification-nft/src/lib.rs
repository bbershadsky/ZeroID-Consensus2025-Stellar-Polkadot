...
impl EmployerVerificationNFT {
    ...
    pub fn mint(env: Env, recipient: Address, metadata_uri: String) -> u64 {
        ...
        persistent.set(&(symbol!("o"), token_id), &recipient);      // owner for token_id
        persistent.set(&(symbol!("m"), token_id), &metadata_uri);   // metadata for token_id
    ...
    pub fn owner_of(env: Env, token_id: u64) -> Address {
        env.storage()
            .persistent()
            .get(&(symbol!("o"), token_id))
            .expect("token not found")
    }

    /// Check if a given address is the owner of a token. Returns true if so, false otherwise.
    pub fn is_owner(env: Env, token_id: u64, addr: Address) -> bool {
        match env.storage().persistent().get::<(soroban_sdk::Symbol, u64), Address>(&(symbol!("o"), token_id)) {
